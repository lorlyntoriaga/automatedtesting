...
const main = async () => {
  ...
  try {
    ...
    const clickProductSearch = await driver.wait(
      until.elementLocated(By.id("autocomplete_0_0"), 4000),
    );
    ...
    const addQuantity = await driver.wait(
      until.elementLocated(By.name("product_qty"), 4000),
    );
    ...
    if (!isChecked) {
      await driver.executeScript("arguments[0].click();", expcb);
    }
    ...
    const year2030 = await driver.wait(
      until.elementLocated(
        By.xpath(
          '//div[contains(@class,"o_date_item_cell") and text()="2030"]',
        ),
      ),
      6000,
    );
    ...
    const saveDetails = await driver.wait(
      until.elementLocated(
        By.xpath(
          '//button[contains(@class,"o_form_button_save") and normalize-space()="Save"]',
        ),
      ),
      6000,
    );
